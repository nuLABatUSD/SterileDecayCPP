...
freqs_ntT::freqs_ntT(int num, double start, double end, double ms, double theta, linspace_and_gl* E, dummy_vars* freqs, double ns, double time, double temp):dep_vars(6*num + 3)
{
    num_bins = num;
    a_start = start;
    a_end = end;
    //eps = new linspace_and_gl(0, a_end * (ms + 1) / 2, num, 0);
    eps = E;
    sterile_mass = ms;
    mixing_angle = theta;
...
freqs_ntT::freqs_ntT(int num, double ms, double theta):dep_vars(6 * num + 3)
{
    num_bins = num;
    sterile_mass = ms;
    mixing_angle = theta;
...
double freqs_ntT::get_ind_neutrino_density(int identifier, double a){
    ...
    return eps->integrate(integrand);
}
...
dummy_vars* freqs_ntT::get_separations(){
    ...
    dummy_vars* overfull = new dummy_vars(9 * num_bins);
    double index = 0;
    ...
    dummy_vars* reduced = new dummy_vars(index - num_switches);
    for(int i = 0; i < index - num_switches; i++){
        reduced->set_value(i, overfull->get_value(i + 9 * num_bins - index + num_switches));
    }
